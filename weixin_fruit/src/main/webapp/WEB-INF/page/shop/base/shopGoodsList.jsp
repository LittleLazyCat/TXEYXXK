...
<div class="pd-20">
	...
	<div class="cl pd-5 bg-1 bk-gray"> <span class="l">  <a class="btn btn-primary radius" href="javascript:;" onclick="addGoods('添加商品','${ctx}/weixinShopGoodsController/toAddShopGoods','700','300')" ><i class="Hui-iconfont">&#xe600;</i> 录入</a> <a class="btn btn-primary radius" href="javascript:;" onclick="addComment('添加反馈意见','${ctx}/comment/toAddComment','700','300')" ><i class="Hui-iconfont">&#xe600;</i> 导出</a> </span>  
	</div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			<thead>
				<tr class="text-c">
					...
					<th width="180">录入顺序</th>
					<th width="180">商品图片</th>
					<th width="180">商品类别</th>
					<th width="180">商品规格</th>
					<th width="180">商品状态</th>
					<th width="180">商品原价</th>
					<th width="180">商品现价</th>
					<th width="180">折扣</th>
					<th width="180">销售数量</th>
					<th width="180">评价数量</th>
					<th width="180">好评数量</th>
					<th width="180">差评数量</th>
					<th width="180">卖家ID</th>
					<th width="120">操作</th>
				</tr>
			</thead>
			<tbody>
			<c:forEach items="${data }" var="d">
				<tr class="text-c">
					...
					<td>${d.TITLE }</td>
					<td>${d.SEQUENCE }</td>
					<td><img width="100" height="50" src="${ctx }/goods_img/${d.TITLE_IMG }"/></td>
					<td>${d.catname }</td>
					<td>${d.standard }</td>
					<td>${d.outStock }</td>
					<td>${d.price }</td>
					...
					<td>${d.sellCount }</td>
					<td class="f-14">
						<a title="编辑" href="javascript:;" onclick="updGoods('修改商品','${ctx}/weixinShopGoodsController/toUpdShopGoods?id=${d.id }','1')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
						<a title="删除" href="javascript:;" onclick="delGoods(this,'${d.id}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
				</tr>
			</c:forEach>
			</tbody>
...
function delGoods(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.get("${ctx}/weixinShopGoodsController/delGoods/"+id,function(data){
			if(data.ok) {
...
function addGoods(title,url,w,h){
...
function updGoods(title,url,w,h){
	var index = layer.open({
		type: 2,
		title: title,
		content: url
