		...
		<tbody>
			<c:forEach items="${data }" var="d">
				<tr class="text-c">
					...
					<td>${d.name }</td>
					<td class="td-status">
						<c:if test="${d.status eq 1}"><span class="label label-success radius">已启用</span></c:if>
						<c:if test="${d.status eq 0}"><span class="label label-defaunt radius">已停用</span></c:if>
					</td>
						
					<td class="f-14">
						<a style="text-decoration:none" class="ml-5" onclick="tosend('邮件发送','${ctx }/email/toSend/${d.id}','700','500')" href="javascript:;" title="发送邮件"><i class="Hui-iconfont">&#xe6de;</i></a> 
						<shiro:hasPermission name="sys:email:delete">
							<a style="text-decoration:none" class="ml-5"  href="${ctx }/email/delete/?id=${d.id}" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</shiro:hasPermission>
					</td>
				</tr>
			</c:forEach>
